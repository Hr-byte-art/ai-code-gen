...
import com.wjh.aicodegen.langgraph4j.node.concurrent.*;
 ...
 * 代码生成工作流
 * <p>
 * PS: 是完整的工作流,只不过当前的图片收集的 并发 是使用的 Langgraph4j 的 子图 方法实现的。
 *     相对于 CompletableFuture 创建工作流的方式，
 *     这个工作流更适合有不止一个子节点的，当前我的这个项目实际每一个 子图 只有一个节点，所以是及其不建议使用这个的 ， 而且 实现很麻烦，工作量大，代码复杂。
